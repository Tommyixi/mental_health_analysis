...
bottom_side_total_3 <- grep("^b0([1-3])",colnames(MH))
bottom_side_total_4 <- grep("^b0([4-9])",colnames(MH))
bottom_side_total_5 <-grep("(^d0([1-9])|^d1([0]))",colnames(MH)) 
...
create_heatmap_total = function(created_matrix){
  ...
  for(row in 1:nrow(created_matrix)) {
    if(counter <= length(right_side_ethn)){
      for(col in 1:ncol(created_matrix)) {
        created_matrix[row,col] = round(mean(get(colnames(created_matrix)[col],MH)[MH$ethn_shtdesc==rownames(created_matrix)[row]], na.rm=TRUE), digits =1)
      }
    counter <- counter + 1
    }
    if((counter <= (length(right_side_ethn) + length(right_side_gender)) && counter > length(right_side_ethn)) ){
      for(col in 1:ncol(created_matrix)) {
      created_matrix[counter,col] = round(mean(get(colnames(created_matrix)[col],MH)[MH$gender==rownames(created_matrix)[counter]], na.rm=TRUE), digits=1)
      }
    counter <- counter + 1 
    }
    if((counter <= (length(right_side_ethn) + length(right_side_gender) + length(right_side_sexuality)) && counter > length(right_side_ethn) + length(right_side_gender)) ){
      for(col in 1:ncol(created_matrix)) {
        created_matrix[counter,col] = round(mean(get(colnames(created_matrix)[col],MH)[MH$g02==rownames(created_matrix)[counter]], na.rm=TRUE), digits = 1)
      }
    counter <- counter + 1 
    }    
  }
  ...
  heatmap.2(as.matrix(created_matrix), cellnote = created_matrix, density.info="none",trace="none", notecol="black", dendrogram="none", Colv=FALSE, Rowv= FALSE, margins = c(11,15) )
}
...
create_heatmap_total(generate_total_matrix(bottom_side_total_3))
create_heatmap_total(generate_total_matrix(bottom_side_total_4))
create_heatmap_total(generate_total_matrix(bottom_side_total_5))
