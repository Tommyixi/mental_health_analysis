...
create_heatmap_ethn = function(created_matrix){
  for(row in 1:nrow(created_matrix)) {
    for(col in 1:ncol(created_matrix)) {
      created_matrix[row,col] = mean(get(colnames(created_matrix)[col],MH)[MH$ethn_shtdesc==rownames(created_matrix)[row]], na.rm=TRUE)
    }
  }
print(heatmap(created_matrix, Rowv=NA, Colv=NA, scale = "none", col = cm.colors(200), margins=c(5,10)))  
}
...
create_heatmap_gender = function(created_matrix){
  for(row in 1:nrow(created_matrix)) {
    for(col in 1:ncol(created_matrix)) {
      created_matrix[row,col] = mean(get(colnames(created_matrix)[col],MH)[MH$gender==rownames(created_matrix)[row]], na.rm=TRUE)
    }
  }
print(heatmap(created_matrix, Rowv=NA, Colv=NA, scale = "none", col = cm.colors(200), margins=c(5,10)))  
}
...
create_heatmap_mhealth = function(created_matrix){
  for(row in 1:nrow(created_matrix)) {
    for(col in 1:ncol(created_matrix)) {
      created_matrix[row,col] = mean(get(colnames(created_matrix)[col],MH)[MH$a06==rownames(created_matrix)[row]], na.rm=TRUE)
    }
  }
...
create_heatmap_sexuality = function(created_matrix){
  for(row in 1:nrow(created_matrix)) {
    for(col in 1:ncol(created_matrix)) {
      created_matrix[row,col] = mean(get(colnames(created_matrix)[col],MH)[MH$g02==rownames(created_matrix)[row]], na.rm=TRUE)
    }
  }
